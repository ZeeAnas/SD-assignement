...
router.post('/add', validateParticipant, async(req, res) => {
    ...
    try {
        ...
        return res.status(201).jsend.sucess ({ statusCode: 201, result: 'Participant created successfully'});
    } catch (error) {
        return res.status(500).jsend.error({ message: 'Failed to create participant'});
    }
});
...
router.get('/work/:email', async (req, res) => {
    ...
    try {
        ...
        return res.jsend.sucess({ statusCode: 200, result: { companyname, salary, currency } });
    } catch (error) {
        console.error(error);
...
router.delete('/:email', async (req, res) => {
    ...
    try {
       ...
       return res.jsend.success({ statusCode: 200, result: 'Participant deleted successfully'});
        ...
        console.error(error);
        return res.jsend(500).jsend.error({ message: 'Failed to delete participant'});
